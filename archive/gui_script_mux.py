...
# =============================================================================
# PalmSens MethodSCRIPT Parser Integration (unchanged base, but we now also read Channel/time)
# =============================================================================
...
# =============================================================================
# Helper: SI formatting (unchanged)
# =============================================================================
...
# =============================================================================
# SerialMeasurementRunner (UPDATED minimally)
# - Accepts script_text override (manual entry)
# - More robust connect handshake that ignores stray P lines
# - Parses and stores Channel (ea) + supports multi-channel CSV
# =============================================================================

class SerialMeasurementRunner:
    def __init__(self, script_path, log_callback=print, script_text: Optional[str] = None):
        self.script_path = Path(script_path)
        self.script_text_override = script_text
    ...
    def find_device_port(self):
        ...
        def candidate_key(dev: str):
            # deprioritize pump port if it is in the list
    ...
    def _drain_input(self, max_seconds: float = 0.7):
        """Drain any pending lines to avoid mid-stream leftovers between runs."""
    ...
    def _read_banner(self, max_seconds: float = 2.0) -> Optional[str]:
        """
        Read until we see a banner like 'tespico...' and IGNORE stray packet lines.
        """
        if not self.connection:
            return None
        t0 = time.time()
        while time.time() - t0 < max_seconds:
            ...
            if not s:
                continue
            # Ignore packets (this is the run-#2 failure mode you saw)
            if s.startswith('P'):
                continue
            if 'tespico' in s.lower():
                return s
            # Sometimes the device echoes single chars on wake; keep reading
        return None
    ...
    def connect(self, port=None):
        ...
        try:
            ...
            # Drain any leftovers
            self._drain_input(max_seconds=0.5)

            # Request ID/banner
            self.connection.write(b't\n')
            self.connection.flush()

            banner = self._read_banner(max_seconds=2.0)
            if banner:
                self.log(f"Device responded: {banner}")
                # Drain again so the next read starts cleanly
                self._drain_input(max_seconds=0.2)
                return True

            # Fallback: read one line (original behavior)
    ...
    def run_script(self, script: str):
        if not self.connection:
            ...
            return False
        try:
            # Critical: drain RX so we don't treat leftover packets as the next run's header
            self._drain_input(max_seconds=0.5)
            ...
            while self.is_running:
                try:
                    ...
                    # Most reliable end marker is '*'
                    if text == '*':
                        saw_terminator = True
                        self.log("\nMeasurement completed")
                        break

                    # Keep old sentinels as well
            ...
            if not saw_terminator and not self.is_running:
                self.log("Measurement stopped by user.")
            elif not saw_terminator:
                # Not fatal, but helpful diagnostic
                self.log("Warning: did not see '*' terminator; run may have ended early or desynced.")
            return True
    ...
    def parse_data_line(self, line: str):
        """
        Parse a MethodSCRIPT data package line and store:
          - potential (V)
          - current (µA)   (convert from A -> µA)
          - channel (int)  (from 'ea' variable if present)
          - time_s (optional) (from 'eb' if present)
        """
        ...
        try:
            ...
            for var in package:
                if var.id in ['ab', 'da']:
                    ...
                    data_point['current'] = var.value * 1e6  # A -> µA
                elif var.id == 'ea':
                    # Channel is dimensionless; treat as integer-ish
                    try:
                        data_point['channel'] = int(round(var.value))
                    except Exception:
                        data_point['channel'] = var.value
                elif var.id == 'eb':
                    data_point['time_s'] = var.value

            if 'potential' in data_point and 'current' in data_point:
                if 'channel' not in data_point:
                    data_point['channel'] = 0
    ...
    def save_data_to_csv(self):
        ...
        # Include channel/time if present
        has_channel = any('channel' in dp for dp in self.data_points)
        has_time = any('time_s' in dp for dp in self.data_points)

        fieldnames = ['potential', 'current']
        if has_channel:
            fieldnames.append('channel')
        if has_time:
            fieldnames.append('time_s')

        with open(csv_filename, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)

            header_row = {'potential': 'Potential (V)', 'current': 'Current (µA)'}
            if has_channel:
                header_row['channel'] = 'Channel'
            if has_time:
                header_row['time_s'] = 'Time (s)'
    ...
    def execute(self):
        ...
        # Load script either from path or override text (manual entry)
        try:
            if self.script_text_override is not None:
                script = self.script_text_override
            else:
...
# =============================================================================
# GUI (keep original structure; add ONE new tab for manual MethodSCRIPT entry)
# =============================================================================
...
class ElectrochemGUI:
    ...
    def setup_gui(self):
        ...
        # NEW (minimal): manual MethodSCRIPT entry tab, does not change existing workflow
        self.manual_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.manual_frame, text="Manual MethodSCRIPT")
    ...
    # ----------------------------------------------------------------------------
    # NEW: Manual MethodSCRIPT tab
    # ----------------------------------------------------------------------------

    def setup_manual_tab(self):
        outer = ttk.Frame(self.manual_frame)
        outer.pack(fill='both', expand=True, padx=10, pady=10)

        controls = ttk.Frame(outer)
        controls.pack(side='top', fill='x', pady=(0, 8))

        ttk.Button(controls, text="Load .ms/.txt", command=self.manual_load_script).pack(side='left', padx=4)
        ttk.Button(controls, text="Save to .ms", command=self.manual_save_script).pack(side='left', padx=4)
        ttk.Button(controls, text="Run Manual Now", command=self.run_manual_immediately).pack(side='left', padx=12)

        ttk.Label(controls, text="(Paste MethodSCRIPT below; this does not affect the queue.)").pack(side='left', padx=10)

        self.manual_text = scrolledtext.ScrolledText(outer, wrap=tk.NONE, height=26, font=('Courier', 11))
        self.manual_text.pack(fill='both', expand=True)

    def manual_load_script(self):
        filepath = filedialog.askopenfilename(
            title="Select a MethodSCRIPT file",
            filetypes=(("MethodSCRIPT", "*.ms"), ("Text files", "*.txt"), ("All files", "*.*"))
        )
        if not filepath:
            return
        try:
            ...
            self.update_script_preview(content)
            self.notebook.select(self.manual_frame)
        except Exception as exc:
            messagebox.showerror("Load Error", f"Failed to load script:\n{exc}")

    def manual_save_script(self):
        content = self.manual_text.get("1.0", tk.END).strip()
        if not content:
            messagebox.showwarning("Empty", "Manual script text is empty.")
            return
        date_folder = self.base_path / datetime.now().strftime('%Y-%m-%d')
        date_folder.mkdir(exist_ok=True)
        filename = filedialog.asksaveasfilename(
            title="Save Manual Script",
            defaultextension=".ms",
            initialdir=str(date_folder),
            initialfile="manual.ms",
            filetypes=(("MethodSCRIPT", "*.ms"), ("All files", "*.*"))
        )
        if not filename:
            return
        try:
            with open(filename, "w", encoding="utf-8") as f:
                f.write(content + "\n")
            messagebox.showinfo("Saved", f"Saved manual script:\n{filename}")
        except Exception as exc:
            messagebox.showerror("Save Error", f"Failed to save:\n{exc}")

    def run_manual_immediately(self):
        script = self.manual_text.get("1.0", tk.END).strip()
        if not script:
            messagebox.showwarning("Empty", "Paste a MethodSCRIPT into the manual editor first.")
            return
        # Keep consistent behavior with existing runs: save it (so naming + provenance are consistent)
        try:
            filepath, filename = self.save_script_file("MANUAL", script)
        except Exception as e:
            messagebox.showerror("File Error", f"Failed to save MANUAL script: {e}")
            return
        # Run using the existing immediate-run pipeline
        self.run_script_immediately("MANUAL", script, script_path_override=Path(filepath))

    # ----------------------------------------------------------------------------
    # Plotter tab (UPDATED to plot multiple channels if present)
    # ----------------------------------------------------------------------------
    ...
    def plot_data(self, csv_path):
        ...
        channel_col = self._find_column(df, ("Channel", "channel"))
        ...
        try:
            ...
            if channel_col:
                # Plot each channel in a different color (matplotlib default cycle)
                # Keep order stable for legend.
                try:
                    channels = sorted([c for c in df[channel_col].dropna().unique()])
                except Exception:
                    channels = list(df[channel_col].dropna().unique())

                for ch in channels:
                    sub = df[df[channel_col] == ch]
                    self.ax.plot(sub[potential_col], sub[current_col], label=f"Ch {ch}")
    ...
    # ----------------------------------------------------------------------------
    # Immediate run (UPDATED: optional script_path_override so MANUAL uses its saved .ms name)
    # ----------------------------------------------------------------------------

    def run_script_immediately(self, technique, script, script_path_override: Optional[Path] = None):
        if self.is_running:
            messagebox.showwarning(
                "Busy",
                "Another measurement is currently running. Stop it or wait for it to finish before starting a new run."
            )
            return

        try:
            if script_path_override is not None:
                filepath = script_path_override
                filename = script_path_override.name
            else:
        ...
        def worker():
            ...
            runner = None
            try:
                # Use script text override so "manual" runs exactly what is in the editor
                runner = SerialMeasurementRunner(Path(filepath), log_callback=self.log_message, script_text=script)
    ...
    # ----------------------------------------------------------------------------
    # Queue persistence / execution (UNCHANGED)
    # ----------------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------------
    # Pump tab (UNCHANGED; omitted here for brevity in this explanation)
    # You provided a long pump section; keep it exactly as-is in your local file.
    # ----------------------------------------------------------------------------
    # IMPORTANT:
    # Paste your existing setup_pump_tab() and all pump_* methods below this comment
    # EXACTLY as they are in your current file.
    #
    # To keep this answer usable as a single file, I’m including your pump section
    ...
    # ---- Pump queue helpers + pump_threaded + pump actions (unchanged from your file) ----
    # (Keeping everything you provided as-is; no changes needed for manual/script/plot.)
