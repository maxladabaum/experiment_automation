...
from .. import analysis_processing as fallback_processing


def apply_smoothing(i: np.ndarray, smooth_window: int, smooth_polyorder: int) -> np.ndarray:
    if savgol_filter is None:
        return fallback_processing.apply_smoothing(i, smooth_window, smooth_polyorder)
    w = int(smooth_window)
    if w >= len(i):
        w = max(3, (len(i) // 2) * 2 + 1)
    if w % 2 == 0:
...
def find_peak_candidates(
    ...
    if find_peaks is None:
        return fallback_processing.find_peak_candidates(
            i_smooth,
...
def rotate_offset_using_prominent_bracketing_minima(
    ...
    if find_peaks is None:
        return fallback_processing.rotate_offset_using_prominent_bracketing_minima(
            voltage,
            y,
            peak_idx,
            search_window_v=search_window_V,
        )
    v = np.asarray(voltage, dtype=float)
    y = np.asarray(y, dtype=float)
