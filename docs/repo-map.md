...
|-- pump_gui.py                    # Pump GUI + PumpCtrl used by gui/app.py
|-- PumpCommServer.dll             # Pump COM server dependency
|-- optimizer/
|   |-- bo_configs/                # BO configs and local path template
|   `-- bo_simulations/            # Ignored simulation outputs
|-- tools/
|   `-- legacy/
|       `-- pump/
|           |-- centris_pure.py
|           `-- sample_to_waste_ex.py
|
|-- gui/
|   |-- __init__.py
