...
class SessionBar:
    ...
    def _build(self):
        ...
        ttk.Button(exp, text="Choose Experiment", command=self._on_choose_experiment).grid(
            row=0, column=2, columnspan=2, sticky="e", padx=5, pady=2
        )
    ...
    def _on_choose_experiment(self):
        if not self._mgr.has_session:
            messagebox.showerror("No Session", "Choose or start a session first.")
            return
        path = filedialog.askdirectory(
