...
start "Experiment Automation" "%GIT_BASH_BIN%" --login -i -c "cd '%APPDIR:\=/%' && echo '========================================' && echo '[INFO] Experiment Automation Launcher' && echo '[INFO] App folder: %APPDIR:\=/%' && echo '[INFO] Git Bash: %GIT_BASH_BIN:\=/%' && if [ -f venv_gui/Scripts/activate ]; then echo '[INFO] Virtual environment detected: venv_gui'; source venv_gui/Scripts/activate; else echo '[INFO] No repo virtual environment detected. Using Git Bash python environment.'; fi && echo '[INFO] Launching app in 2 seconds...' && echo '========================================' && sleep 2 && python -X faulthandler -m main; ec=$?; if [ $ec -ne 0 ]; then echo; echo 'Launcher detected an error (exit' $ec '). Press Enter to close...'; read -r; fi"
